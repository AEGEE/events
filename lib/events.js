...
exports.listMostRecentEvents = async (req, res) => {
    const queryObj = {
        ...
        attributes: [
            'organizing_bodies',
            [Sequelize.fn('MAX', Sequelize.col('starts')), 'latest_event']
        ]
    };

    if (req.query.ends) queryObj.merge({ where: { ends: { [Sequelize.Op.lte]: moment(req.query.ends, 'YYYY-MM-DD').endOf('day').toDate() } } });
